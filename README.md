...
- `data/Stats_Extended_2013_Beyond/` contains legacy Excel files with player totals, per-game, per-36, per-100, and advanced statistics for 2013-14 and 2014-15.
- `data/processed/` is generated by the modern hoopR pipeline and covers 2013 through the latest available season.
...
### Refresh Modern NBA Data

The Docker build refreshes modern player and team season statistics from 2013 through the latest season available from hoopR.

For a local refresh:

```bash
Rscript scripts/refresh_modern_data.R
Rscript scripts/validate_modern_data.R
```

Override the season range when needed:

```bash
NBA_START_SEASON=2023 NBA_END_SEASON=2026 Rscript scripts/refresh_modern_data.R
```

Generated files are written to `data/processed/`. See [Modern NBA Data Source](docs/MODERN_DATA_SOURCE.md) for source, identity, recognition, and usage notes.
